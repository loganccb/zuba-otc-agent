...
1. ENQUIRY: Customer hasn't given you the hard currency or volume yet.
   ...
   - Once you have both, say only: "Got it. One moment."
...
def handle_message(phone_number: str, message: str) -> str:
    ...
    # Quote message - rate just fetched
    if trade.state == TradeState.RATE_QUOTED and not trade.trade_summary_sent:
        just_quoted = trade.quote_time and (datetime.utcnow() - trade.quote_time).total_seconds() < 10
        if just_quoted:
