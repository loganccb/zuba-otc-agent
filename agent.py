    ...
    if trade is None or trade.state == TradeState.ENQUIRY:
        ...
        if trade.quote_time:
            if is_rate_expired(trade.quote_time):
                ...
                expiry_dt = trade.quote_time + timedelta(seconds=600)
                expiry_info = expiry_dt.strftime("%H:%M UTC")
        ...
        # Pre-calculate local amount for use in trade summary only
        local_amount = ""
...
Local amount (for trade summary only - do not show in quote): {local_amount or '-'}
...
- Once you have both, present the rate immediately using the format in RATE_QUOTED below.
- Do NOT ask clarifying questions about currency variant (e.g. do not ask "USD cash or USDT?"). Use exactly the currency they stated.
- Do NOT ask for their target rate. Just quote our rate.
- Do NOT say you are fetching the rate. The rate is already in your context - present it immediately.

### RATE_QUOTED
Your entire reply must be exactly these three elements - no introduction, no amounts, no notes, nothing else whatsoever:

Rate: [CUSTOMER RATE] [LOCAL CURRENCY] per [HARD CURRENCY]
Expires at: [QUOTE EXPIRY from context above]
...
The rate is always available in your context. Never add notes about rates not being loaded, live, or available. Never add calculated amounts. Never add any text outside these three lines.
...
Account Details
Name: {ZUBA_ACCOUNT["account_name"]}
Account number: {ZUBA_ACCOUNT["account_number"]}
Bank: {ZUBA_ACCOUNT["bank_name"]}

Rate is locked. Do not send funds until you have received this confirmation.
...
SWIFT/IBAN:
Address:
Amount: [HARD CURRENCY] [AMOUNT]

### SUMMARY_POSTED
Once the client has sent back their beneficiary details, output the trade summary in exactly this format, then signal [STATE:SUMMARY_POSTED]:

TRADE SUMMARY

Trade ID:         [TRADE_ID]
Output Currency:  [HARD CURRENCY]
Output Amount:    [HARD CURRENCY AMOUNT]
Input Currency:   [LOCAL CURRENCY]
Input Amount:     [LOCAL CURRENCY AMOUNT]
Exchange Rate:    [CUSTOMER RATE]
Name of customer: [COUNTERPARTY NAME if given, else 'unknown']

Beneficiary Name: [NAME]
Address:          [ADDRESS]
Bank name:        [BANK]
SWIFT/IBAN:       [SWIFT OR IBAN]
BIC:              [BIC if provided, else omit this line]
