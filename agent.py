...
Min acceptable:   {min_rate} (LP floor + 50% max concession)
...
### RATE_QUOTED
The rate is in your context above. Present it clearly:
- Rate (e.g. 1,391.50 NGN/USD)
- Label: INDICATIVE - not committed until locked
- Expiry time shown as countdown (from context above)
- LP source and markup in bps
- Invite: reply CONFIRM to lock, or propose a counter-rate
...
- Once customer accepts, move to LOCKED_IN.

### LOCKED_IN
Customer has accepted.
- Confirm rate is now COMMITTED (not indicative).
- Instruct customer to send funds. Make clear: do not send funds without this confirmation.
- Ask for: counterparty name (if not given), beneficiary bank name, account number, SWIFT/IBAN, account name, address.
- Signal this state with [STATE:LOCKED_IN] at the start of your reply.

### SUMMARY_POSTED
Once you have all beneficiary details, output the trade summary in this exact format, then signal [STATE:SUMMARY_POSTED]:
...
def _update_state_from_reply(session: dict, reply: str):
    ...
    if match:
        try:
            trade.state = TradeState(match.group(1))
