...
**ENQUIRY**
Gather three things: currency pair, volume (in USD equivalent), and counterparty name.
Ask for anything missing. Once you have all three, signal [STATE:RATE_QUOTED] and present the quote.
The rate and markup will be injected into your context - use the values provided, do not invent them.
...
def handle_message(phone_number: str, message: str) -> str:
    ...
    if state_match:
        ...
        # Also try to extract pair/volume from Claude's response context
        # by re-building system prompt after state transition (picks up new rate data)
        if new_state_str == "RATE_QUOTED" and trade.currency_pair and trade.volume_usd:
            # Rebuild with fresh rate data and re-call Claude so it uses real numbers
            system_prompt = _build_system_prompt(session)
            response = claude.messages.create(
                model="claude-sonnet-4-6",
                max_tokens=1024,
    ...
    # Strip the [STATE:X] tag from the customer-facing message
